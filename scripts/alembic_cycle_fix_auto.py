...
4) 每次改动都会写 .bak 备份
"""
import os, re, io, glob, sys, shutil, datetime, ast
...
def parse_down_expr(expr: str) -> List[str]:
    ...
    if expr.startswith("Union") or expr.startswith("(") or expr.startswith("[") or expr.startswith("{"):
        pass
    try:
        # 尝试用 ast 解析右边表达式
...
def rewrite_down(meta: Dict[str, dict], rev: str, new_downs: List[str]) -> bool:
    ...
    if new_txt!=txt:
        ...
        io.open(info["path"], "w", encoding="utf-8").write(new_txt)
        info["text"]=new_txt
    ...
    while True:
        ...
        for cyc in cycles:
            ...
            for r in cyc:
                ...
                new_downs = [d for d in downs if meta.get(d) and (meta[d]["date"]<=meta[r]["date"])]
                # 确保至少有一路能通向最老
                if r!=oldest and oldest not in new_downs:
                    new_downs = [oldest] if not new_downs else list(dict.fromkeys(new_downs+[oldest]))
                if new_downs!=downs:
    ...
    if not changes:
        ...
        for cyc, patches in changes:
            ...
            for r,new_downs in patches:
                print(f"   * patched {r}: down_revision = {tuple(new_downs) if len(new_downs)>1 else (new_downs[0] if new_downs else None)}")
    print("\nNext:")
