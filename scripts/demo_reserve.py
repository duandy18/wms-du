...
from app.db.base import init_models; init_models()
...
async def run(store_id:int, item_id:int, qty:int, db:str):
    ...
    async with ses() as s:
        new_reserved = await ChannelInventoryService.adjust_reserved(s, store_id=store_id, item_id=item_id, delta=qty)
        await StoreService.refresh_channel_inventory_for_store(s, store_id=store_id, item_ids=[item_id], dry_run=False)
