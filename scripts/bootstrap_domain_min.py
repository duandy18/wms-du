...
# Phase 4E：location_id 退场；这里不再创建 locations，也不再触碰 stocks
SQL_ENSURE_ITEM = """
...
async def _run(store_name: str, item_ids: list[int], db_url: str) -> None:
    engine = create_async_engine(db_url, future=True)
    async_session = sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)

    async with async_session() as s:
        # 1) 维度 & 基础行（Phase 4E：warehouse + items）
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Bootstrap minimal domain: warehouse/items/store/bind + shadow refresh (Phase 4E lot-world; no legacy stocks/batches)"
    )
