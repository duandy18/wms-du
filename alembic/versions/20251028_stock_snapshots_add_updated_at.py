...
def upgrade():
    ...
    if not _col_exists(conn, "stock_snapshots", "updated_at"):
        op.add_column("stock_snapshots", sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True))
    if not _col_exists(conn, "stock_snapshots", "created_at"):
        op.add_column("stock_snapshots", sa.Column("created_at", sa.DateTime(timezone=True), nullable=True))
