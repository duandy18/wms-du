...
def upgrade():
    ...
    has_uc_stocks = conn.scalar(sa.text("""
    ...
    has_ledger_loc = conn.scalar(sa.text("""
        SELECT 1 FROM information_schema.columns
    ...
    has_uc_ledger = conn.scalar(sa.text("""
    ...
    has_batches_loc = conn.scalar(sa.text("""
        SELECT 1 FROM information_schema.columns
    ...
    has_batches_wh = conn.scalar(sa.text("""
        SELECT 1 FROM information_schema.columns
    ...
    has_uc_batches = conn.scalar(sa.text("""
    ...
    has_uc_snap = conn.scalar(sa.text("""
...
def downgrade():
    ...
    has_uc_snap = conn.scalar(sa.text("SELECT 1 FROM pg_constraint WHERE conname='uq_snap_date_wh_item_code'"))
    if has_uc_snap:
        op.drop_constraint("uq_snap_date_wh_item_code", "snapshots", type_="unique")
    has_uc_batches = conn.scalar(sa.text("SELECT 1 FROM pg_constraint WHERE conname='uq_batches_item_code'"))
    if has_uc_batches:
        op.drop_constraint("uq_batches_item_code", "batches", type_="unique")
    has_uc_ledger = conn.scalar(sa.text("SELECT 1 FROM pg_constraint WHERE conname='uq_ledger_idem_reason_refline_item_code_wh'"))
    if has_uc_ledger:
        op.drop_constraint("uq_ledger_idem_reason_refline_item_code_wh", "stock_ledger", type_="unique")
    has_uc_stocks = conn.scalar(sa.text("SELECT 1 FROM pg_constraint WHERE conname='uq_stocks_item_wh_batch'"))
    if has_uc_stocks:
