...
def _idx_exists(conn, name):
    return bool(conn.execute(sa.text(
        "SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname=:n LIMIT 1"
    ), {"n": name}).scalar())
...
def upgrade():
    ...
    if not _idx_exists(conn, "ix_batches_fefo"):
        op.execute("CREATE INDEX ix_batches_fefo ON batches (item_id, location_id, expiry_date, id)")
    # Ledger 时间线：stock 维度
    if not _idx_exists(conn, "ix_stock_ledger_stock_time"):
        op.execute("CREATE INDEX ix_stock_ledger_stock_time ON stock_ledger (stock_id, occurred_at)")
