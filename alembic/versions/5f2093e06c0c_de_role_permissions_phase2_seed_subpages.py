# alembic/versions/5f2093e06c0c_de_role_permissions_phase2_seed_subpages.py
"""de_role_permissions_phase2_seed_subpages

Revision ID: 5f2093e06c0c
Revises: b634b959e15e
Create Date: 2026-04-05 15:29:06.186341
...
revision: str = "5f2093e06c0c"
down_revision: Union[str, Sequence[str], None] = "b634b959e15e"
...
def upgrade() -> None:
    # 1) seed 二级页面
    # 当前 page code 仍沿用 wms.* 命名空间；
    # 真实领域用 domain_code 表达；
    # 二级页面默认继承一级页面权限：inherit_permissions=true, read/write_permission_id=NULL
    op.execute(
        """
...
def downgrade() -> None:
    # 1) 先把 route_prefix 挂回 phase1 的一级页面
    op.execute(
        """
        WITH mapping(route_prefix, page_code) AS (
            VALUES
