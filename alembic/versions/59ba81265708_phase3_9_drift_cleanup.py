...
def upgrade() -> None:
    ...
    # event_store（仅当表存在时才执行）
    if insp.has_table("event_store", schema="public"):
        op.alter_column(
            "event_store",
            "topic",
    ...
    # locations：索引 + 外键，都做好防御
    op.execute(
        """
        DO $$
        BEGIN
          BEGIN
            CREATE INDEX IF NOT EXISTS ix_locations_warehouse_id
              ON locations (warehouse_id);
          EXCEPTION
            WHEN SQLSTATE '55006' THEN
              -- object in use（pending trigger events）：跳过创建，不阻断迁移
              NULL;
          END;

          BEGIN
            CREATE INDEX IF NOT EXISTS ix_locations_wh
              ON locations (warehouse_id);
          EXCEPTION
            WHEN SQLSTATE '55006' THEN
              NULL;
          ...
          -- 外键：同样可能因为 pending trigger events 被卡住，这里也软处理
          BEGIN
            IF NOT EXISTS (
              SELECT 1
              FROM pg_constraint c
              JOIN pg_class t ON t.oid = c.conrelid
              WHERE c.conname = 'fk_locations_warehouse'
          ...
          EXCEPTION
            WHEN SQLSTATE '55006' THEN
              -- 若当前事务里 locations 有挂起事件，就先放过 FK 创建
              NULL;
          END;
        END$$;
        """
