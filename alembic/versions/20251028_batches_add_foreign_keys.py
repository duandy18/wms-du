...
def upgrade():
    ...
    if not _fk_exists(conn, "batches", FK_ITEM):
        op.create_foreign_key(FK_ITEM, "batches", "items", ["item_id"], ["id"], ondelete="RESTRICT", onupdate="RESTRICT")
    if not _fk_exists(conn, "batches", FK_LOC):
        op.create_foreign_key(FK_LOC, "batches", "locations", ["location_id"], ["id"], ondelete="RESTRICT", onupdate="RESTRICT")
    if not _fk_exists(conn, "batches", FK_WH):
        op.create_foreign_key(FK_WH, "batches", "warehouses", ["warehouse_id"], ["id"], ondelete="RESTRICT", onupdate="RESTRICT")
