...
def upgrade():
    ...
    if "order_state_snapshot" not in insp.get_table_names(schema="public" if not sqlite else None):
        op.create_table(
            ...
            sa.UniqueConstraint("platform", "shop_id", "order_no", name="uq_order_state_snapshot_key"),
        )
        op.create_index("ix_order_state_snapshot_lookup", "order_state_snapshot", ["platform", "shop_id", "order_no"], unique=True)
