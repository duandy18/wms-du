...
async def _get_qty(item_id: int, location_id: int) -> int:
    async with async_session_maker() as s:
        q = await s.execute(
            text("SELECT qty FROM stocks WHERE item_id=:iid AND location_id=:lid"),
...
async def _ensure_stage_has(item_id: int, target_qty: int, tag: str) -> None:
    ...
    注意：PG 中 ref_line 是 INTEGER，这里统一写入 1。
    """
    async with async_session_maker() as s:
        async with s.begin():
...
async def _do_putaway_sql(item_id: int, qty: int, to_location_id: int, tag: str) -> None:
    ...
    注意：PG 中 ref_line 是 INTEGER，这里分别写入 1/2。
    """
    async with async_session_maker() as s:
        async with s.begin():
            # 扣暂存位
            ...
            await s.execute(
                text(
                    """
                INSERT INTO stock_ledger (stock_id, reason, after_qty, delta, occurred_at, ref, ref_line)
                VALUES (:sid, 'PUTAWAY', :aft, :d, NOW(), :r, :rl)
            """
                ),
                {
                    "sid": int(sid0),
            ...
            """
                    ...
                    {"q": qty, "iid": item_id, "lid": to_location_id},
                )
            ).first()
            if row is None:
                sid1, after1 = (
                ...
                """
                        ...
                        {"iid": item_id, "lid": to_location_id, "q": qty},
                    )
                ).first()
...
async def test_inbound_receive_and_putaway_integrity():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        assert r1.status_code == 200 or _is_idempotent_409(
            r1
        ), f"unexpected receive: {r1.status_code} {r1.text}"
