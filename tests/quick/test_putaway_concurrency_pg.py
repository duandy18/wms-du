    ...
    async with async_session_maker() as s1, async_session_maker() as s2:
        t1 = asyncio.create_task(
            PutawayService.bulk_putaway(
                s1, stage_location_id=stage, target_locator_fn=locator, batch_size=50, worker_id="A"
            )
        )
        t2 = asyncio.create_task(
            PutawayService.bulk_putaway(
                s2, stage_location_id=stage, target_locator_fn=locator, batch_size=50, worker_id="B"
            )
        )
