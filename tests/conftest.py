...
ALEMBIC_SYNC_URL = RAW_URL.replace("+asyncpg", "+psycopg") if RAW_URL.startswith("postgresql+asyncpg") else RAW_URL
...
def apply_migrations():
    ...
    if ALEMBIC_SYNC_URL.startswith("sqlite"):
        sync_eng = create_engine(ALEMBIC_SYNC_URL, future=True, connect_args={"check_same_thread": False, "timeout": 60})
        try:
...
def _make_sync_engine():
    if IS_PG:
        ...
        eng = create_engine(RAW_URL, future=True, connect_args={"check_same_thread": False, "timeout": 60} if IS_SQLITE else {}, pool_pre_ping=True)
    return eng
...
def _fresh_db():
    ...
    if IS_PG:
        with sync_conn() as conn:
            ...
            target = ["stock_snapshots","stock_ledger","stocks","batches","order_items","orders","items","locations","warehouses"]
            present = [t for t in target if t in existing]
            if present:
                conn.execute(text("TRUNCATE TABLE " + ", ".join(f'"{t}"' for t in present) + " RESTART IDENTITY CASCADE"))

        # 最小种子：WH=1、LOC=0/101、ITEM#1=SKU-001（与 smoke 断言一致）
        s = SyncSessionLocal()
        try:
            s.execute(text("INSERT INTO warehouses (id, name) VALUES (1,'WH-TEST') ON CONFLICT (id) DO NOTHING"))
            s.execute(text("INSERT INTO locations (id, name, warehouse_id) VALUES (0,'STAGE',1) ON CONFLICT (id) DO NOTHING"))
            s.execute(text("INSERT INTO locations (id, name, warehouse_id) VALUES (101,'LOC-101',1) ON CONFLICT (id) DO NOTHING"))
            s.execute(text("INSERT INTO items (id, sku, name, unit) VALUES (1,'SKU-001','Item-1','EA') ON CONFLICT (id) DO NOTHING"))
            s.commit()
        finally:
            s.close()
        ...
        with eng.begin() as conn:
            ...
            for tbl in ["stock_snapshots","stock_ledger","stocks","batches","order_items","orders","items","locations","warehouses"]:
                try: conn.exec_driver_sql(f"DELETE FROM {tbl};")
                except Exception: pass
            conn.exec_driver_sql("PRAGMA foreign_keys=ON;")
            conn.exec_driver_sql("INSERT OR IGNORE INTO warehouses (id, name) VALUES (1,'WH-TEST');")
            conn.exec_driver_sql("INSERT OR IGNORE INTO locations (id, name, warehouse_id) VALUES (0,'STAGE',1);")
            conn.exec_driver_sql("INSERT OR IGNORE INTO locations (id, name, warehouse_id) VALUES (101,'LOC-101',1);")
            conn.exec_driver_sql("INSERT OR IGNORE INTO items (id, sku, name, unit) VALUES (1,'SKU-001','Item-1','EA');")
        eng.dispose()
    yield
...
async def engine(db_url):
    ...
    eng = create_async_engine(db_url, future=True, pool_pre_ping=True, poolclass=NullPool, connect_args=connect_args)
    try:
        yield eng
    finally:
        await eng.dispose()
    ...
    async with async_session_maker() as s:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                if tx.is_active: await tx.rollback()
