...
async def test_inbound_receive_and_putaway_integrity():
    ...
    async with async_session_maker() as s:
        tmp_qty = (await s.execute(
            text("select qty from stocks where item_id=1 and location_id=0")
        )).scalar() or 0
        loc_qty = (await s.execute(
        ...
        sum_delta = (await s.execute(
            text("select coalesce(sum(delta),0) from stock_ledger where item_id=1")
        )).scalar() or 0
        ...
        if await _table_has_column(s, "stock_ledger", "after_qty"):
            # 暂存端 after_qty == 3
            after_tmp = (await s.execute(text("""
                select after_qty from stock_ledger
                where item_id=1 and location_id=0 and op='PUTAWAY'
                order by id desc limit 1
            """))).scalar()
            # 目标位 after_qty == 7
            after_loc = (await s.execute(text("""
                select after_qty from stock_ledger
                where item_id=1 and location_id=101 and op='PUTAWAY'
                order by id desc limit 1
            """))).scalar()
            assert after_tmp == 3 and after_loc == 7, f"after_qty mismatch: tmp={after_tmp}, loc101={after_loc}"
...
async def test_expiry_conflict_422():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        # 同批次但不同效期 -> 422
        r2 = await ac.post("/inbound/receive", json={
            "sku": "SKU-001",
            "qty": 1,
            "batch_code": "B-EXP",
            "production_date": "2025-09-01",
