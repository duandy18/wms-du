...
async def test_reserved_adjust_idempotent(session):
    ...
    await StockService().adjust(session=session, item_id=item, location_id=loc, delta=15, reason="INBOUND")
    ...
    v = await session.execute(text("SELECT COALESCE(visible,0) FROM channel_inventory WHERE store_id=:s AND item_id=:i"),
                              {"s": store, "i": item})
