...
async def _sum_inv(session, item):
    r = await session.execute(text("SELECT COALESCE(SUM(qty),0) FROM stocks WHERE item_id=:i"), {"i": item})
    return int(r.scalar() or 0)

async def _sum_ledger(session, item):
    r = await session.execute(text("SELECT COALESCE(SUM(delta),0) FROM stock_ledger WHERE item_id=:i"), {"i": item})
    return int(r.scalar() or 0)
