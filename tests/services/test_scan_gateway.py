...
pytestmark = pytest.mark.skip(
    reason="Legacy v1 /scan gateway test; superseded by v2 scan_orchestrator flow",
)
...
def test_scan_gateway_transaction_success():
    ...
    assert response.json().get("committed") is True

    # 验证库存更新是否成功
    stock = get_stock_by_barcode("ITEM123")  # 查询库存
    assert stock is not None  # 确保库存记录存在
...
def test_scan_gateway_transaction_failure():
    ...
    # 验证返回的错误
    assert response.status_code == 400  # 期待错误响应
