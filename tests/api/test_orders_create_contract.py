...
def test_platform_orders_ingest_minimal_contract(client: TestClient) -> None:
    ...
    - 支持 platform / store_code / ext_order_no + 顶层地址字段 + lines 接入；
    - 返回 JSON 中至少包含：
    ...
    - ref 仍采用 "ORD:{platform}:{store_code}:{ext_order_no}" 格式。
    """
    payload = {
        "platform": "PDD",
        "store_code": "TEST_STORE_ORD",
    ...
    expected_ref = f"ORD:{payload['platform']}:{payload['store_code']}:{payload['ext_order_no']}"
...
def test_platform_orders_ingest_with_no_lines_still_returns_structured_result(
    ...
    assert isinstance(data["unresolved"], list)
    assert isinstance(data["facts_written"], int)
    assert data["ref"].startswith("ORD:PDD:TEST_STORE_ORD:")
