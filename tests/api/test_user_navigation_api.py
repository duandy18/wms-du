...
async def _reset_navigation_registry_state(session: AsyncSession) -> None:
    ...
    """
    await session.execute(text("UPDATE page_registry SET is_active = TRUE WHERE is_active IS DISTINCT FROM TRUE"))
...
def _index_pages(pages: list[dict[str, Any]]) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
...
async def test_my_me_shape_unchanged(client: AsyncClient) -> None:
    ...
    assert set(data.keys()) == {"id", "username", "roles", "permissions"}
...
async def test_my_navigation_admin_baseline_counts(client: AsyncClient) -> None:
    ...
    assert child_count == 27

    assert len(route_prefixes) == 28
