...
from tests.api._helpers_shipping_quote import auth_headers, login, require_env
...
def test_metrics_shipping_quote_failures_collects_quote_calc_reject(client: TestClient) -> None:
    ...
    r = client.post(
        "/shipping-quote/calc",
        headers=h,
        json={"scheme_id": 999999, "dest": {"province": "北京市"}, "real_weight_kg": 1.0, "flags": []},
    )
    assert r.status_code == 422, r.text
    assert r.json()["detail"]["code"] == "QUOTE_CALC_SCHEME_NOT_FOUND"
