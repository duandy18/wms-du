# app/wms/outbound/helpers/pick_tasks_routes_common.py
...
from app.wms.outbound.contracts.pick_tasks import PrintJobOut
...
async def load_order_meta_or_404(session: AsyncSession, *, order_id: int) -> dict:
    ...
    - ✅ 严格语义输出（禁止 COALESCE 伪装）：
      - warehouse_id = actual_warehouse_id（执行仓事实）
      - service_warehouse_id = planned_warehouse_id（服务/归属仓快照）
    """
    row = (
        await session.execute(
            text(
                ...
                SELECT
                  ...
                  o.ext_order_no,
                  o.trace_id,
                  of.actual_warehouse_id  AS warehouse_id,
                  of.planned_warehouse_id AS service_warehouse_id
                FROM orders o
