# app/procurement/contracts/purchase_order_receipts.py
...
class PurchaseOrderReceiptEventOut(BaseModel):
    ...
    当前口径：
    - 直接来自 stock_ledger（reason=RECEIPT）
    - ref 使用 confirmed receipt.ref，而不是旧的 PO-{po_id}
    """

    ref: str = Field(..., description="引用，如 receipt.ref")
