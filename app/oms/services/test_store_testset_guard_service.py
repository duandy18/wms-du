...
from app.oms.services.platform_test_store_service import PlatformTestStoreService


class TestShopTestSetGuardService:
    """
    宇宙边界（唯一真相：platform_test_stores + item_test_sets）：

    目标：测试商品（Test Set）不能进入任何“非测试商铺”，避免污染真实业务宇宙。

    规则（code=DEFAULT）：
    - 非 TEST store：若 FSKU.components 命中 Test Set -> 409（禁止绑定/写入）
    - TEST store：不强制 all-in Test Set（放行；是否 all-in 由 order-sim 等调试入口兜底）
    ...
    def __init__(self, session: AsyncSession):
        ...
        self.test_store = PlatformTestStoreService(session)
