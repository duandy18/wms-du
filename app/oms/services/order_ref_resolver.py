...
def _parse_order_key(order_ref: str) -> Optional[OrderKey]:
    ...
    if len(parts) >= 4 and parts[0].upper() == "ORD":
        ...
        store_code = parts[2].strip()
        ext = ":".join(parts[3:]).strip()
        if platform and store_code and ext:
            return OrderKey(platform=platform, store_code=store_code, ext_order_no=ext)
        return None
...
async def resolve_order_id(session: AsyncSession, *, order_ref: str) -> int:
    ...
    解析规则（硬）：
    1) 纯数字 -> 当作 orders.id
    2) ORD:PLAT:STORE:EXT 或 PLAT:STORE:EXT -> 用 (platform, store_code, ext_order_no) 唯一约束解析
    ...
    if key is not None:
        row = (
            await session.execute(
                ...
                {"p": key.platform, "sid": key.store_code, "ext": key.ext_order_no},
            )
        ).first()
        if row:
            return int(row[0])
        ...
        if os.getenv("WMS_ENV") == "test":
            ...
            ins = (
                await session.execute(
                    text(
                        """
                        INSERT INTO orders(platform, store_code, store_id, ext_order_no)
                        VALUES (:p, :sid, :store_id, :ext)
                        ON CONFLICT (platform, store_code, ext_order_no) DO NOTHING
                        RETURNING id
                        """
                    ),
                    {
        ...
        raise_problem(
            status_code=404,
            error_code="order_not_found",
            message="订单不存在（按 platform/store_code/ext_order_no 查询）。",
            context={"order_ref": s, "platform": key.platform, "store_code": key.store_code, "ext_order_no": key.ext_order_no},
            details=[],
            next_actions=[],
        )
        return 0

    # 3) ext_order_no only -> allow only if unique across all stores
    rows = (
        await session.execute(
            text(
                """
                SELECT id, platform, store_code
                  FROM orders
    ...
    if len(rows) >= 2:
        raise_problem(
            ...
            message="订单引用歧义：仅 ext_order_no 无法唯一定位订单，请传 ORD:PLAT:STORE:EXT。",
            context={"order_ref": s, "ext_order_no": s},
            details=[
                {"id": int(rows[0][0]), "platform": str(rows[0][1]), "store_code": str(rows[0][2])},
                {"id": int(rows[1][0]), "platform": str(rows[1][1]), "store_code": str(rows[1][2])},
            ],
            next_actions=[{"action": "use_full_ref", "label": "使用 ORD:PLAT:STORE:EXT 格式"}],
        )
        return 0
