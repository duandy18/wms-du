...
# app/platform_order_ingestion/taobao/repository.py
...
@dataclass(frozen=True)
class TaobaoAppConfigUpsertInput:
    app_key: str
    callback_url: str
...
async def get_enabled_taobao_app_configs(
    session: AsyncSession,
) -> list[TaobaoAppConfig]:
    stmt = (
        sa.select(TaobaoAppConfig)
        .where(TaobaoAppConfig.is_enabled.is_(True))
        .order_by(TaobaoAppConfig.id.asc())
    )
    result = await session.execute(stmt)
    return list(result.scalars().all())


async def get_enabled_taobao_app_config(
    session: AsyncSession,
) -> Optional[TaobaoAppConfig]:
    rows = await get_enabled_taobao_app_configs(session)
    if not rows:
        return None
    if len(rows) > 1:
        raise ValueError("multiple enabled taobao app configs found")
    return rows[0]


async def require_enabled_taobao_app_config(
    session: AsyncSession,
) -> TaobaoAppConfig:
    row = await get_enabled_taobao_app_config(session)
    if row is None:
        raise ValueError("enabled taobao app config not found")
    return row


async def upsert_current_taobao_app_config(
    session: AsyncSession,
    *,
    data: TaobaoAppConfigUpsertInput,
) -> TaobaoAppConfig:
    app_key = str(data.app_key or "").strip()
    app_secret = str(data.app_secret or "").strip()
    callback_url = str(data.callback_url or "").strip()
    ...
    if sign_method not in {"md5", "hmac"}:
...
async def delete_credential_by_store_platform(
    session: AsyncSession,
    *,
    store_id: int,
    platform: str,
