...
# app/platform_order_ingestion/jd/repository.py
...
async def get_enabled_jd_app_configs(
    session: AsyncSession,
) -> list[JdAppConfig]:
    stmt = (
        sa.select(JdAppConfig)
        .where(JdAppConfig.is_enabled.is_(True))
        .order_by(JdAppConfig.id.asc())
    )
    result = await session.execute(stmt)
    return list(result.scalars().all())


async def get_enabled_jd_app_config(
    session: AsyncSession,
) -> Optional[JdAppConfig]:
    rows = await get_enabled_jd_app_configs(session)
    if not rows:
        return None
    if len(rows) > 1:
        raise ValueError("multiple enabled jd app configs found")
    return rows[0]


async def require_enabled_jd_app_config(
    session: AsyncSession,
) -> JdAppConfig:
    row = await get_enabled_jd_app_config(session)
    if row is None:
        raise ValueError("enabled jd app config not found")
    return row


async def upsert_current_jd_app_config(
    session: AsyncSession,
    *,
    ...
    gateway_url = str(data.gateway_url or "").strip()
