...
from app.api.routers.platform_orders_ingest_schemas import PlatformOrderIngestIn, PlatformOrderIngestOut
...
async def ingest_platform_order(
        ...
        raise HTTPException(
            ...
            detail=make_problem(
                ...
                context={
                    ...
                    "address_preview": _safe_preview(raw.get("address") if isinstance(raw, dict) else None),
                },
            ),
        )
    ...
    # ✅ 订单级风险聚合（用于“可人工继续”提示；不写 binding）
