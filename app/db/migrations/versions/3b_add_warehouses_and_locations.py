...
down_revision: Union[str, Sequence[str], None] = "7f_merge_cycle_fix"
...
def _has_table(name: str) -> bool:
    ...
    try:
        ...
        # 一些后端没有 has_table，退回 get_table_names
...
def upgrade() -> None:
    ...
    if not _has_table("warehouses"):
        op.create_table(
            ...
            # 需要可扩展: code/created_at/updated_at 等
        )
    ...
    if not _has_table("locations"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ...
                ondelete="RESTRICT",  # 如需仓库删除级联库位，可改为 "CASCADE"
            ),
        )
...
def downgrade() -> None:
    # 先 locations 的索引/约束/外键，再表；最后 warehouses
