# app/services/channel_inventory_legacy.py
...
async def ensure_row(session: AsyncSession, *, store_id: int, item_id: int) -> None:
...
async def adjust_reserved(
    ...
    *,
    ...
    async with tx_ctx:
        ...
        # 1) 确保目标行存在
        await ensure_row(session, store_id=store_id, item_id=item_id)
