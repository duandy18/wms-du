...
def _load_credentials(cfg: PlatformOAuthConfig) -> Tuple[str, str]:
...
class UnifiedOAuthService:
    ...
    async def build_auth_url(self, platform: Platform, store_id: int) -> Tuple[str, str]:
        ...
        client_id, _ = _load_credentials(cfg)
    ...
    async def handle_callback(
        ...
        cfg = self._get_config(platform)
        client_id, client_secret = _load_credentials(cfg)
    ...
    def _parse_state(self, state: str) -> Tuple[Platform, int]:
        try:
            _, platform, store_id_str = state.split(".", 2)
            return platform, int(store_id_str)  # type: ignore[return-value]
