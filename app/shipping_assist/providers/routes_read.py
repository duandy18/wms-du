# app/shipping_assist/providers/routes_read.py
...
def register(router: APIRouter) -> None:
    ...
    async def list_shipping_providers(
        ...
        for r in c_rows:
            ...
            by_pid.setdefault(pid, []).append(row_to_contact(r))

        data: List[ShippingProviderOut] = []
        for r in prov_rows:
            pid = int(r["id"])
            contacts = by_pid.get(pid, [])
            data.append(row_to_provider(r, contacts))
    ...
    async def get_shipping_provider(
        ...
        return ShippingProviderDetailOut(ok=True, data=row_to_provider(row, contacts))
